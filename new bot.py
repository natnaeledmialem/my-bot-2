...
CHANNEL_ID = ["@ghhggghhhgg","@works_11w","@aa_11_b1","@samiworkers"] # ቦቱ Admin መሆን አለበት!
ADMIN_ID = 8465808385
...
# የሰዎችን ብር እና መረጃ መመዝገቢያ (ቀላል Database በMemory)
# ማሳሰቢያ፡ ሰርቨሩ Restart ሲሆን ይህ ይጠፋል። ለረጅም ጊዜ ለመጠቀም SQLite መጠቀም ይመረጣል።
users_db = {}

# ተጠቃሚው ቻናሉን መቀላቀሉን ቼክ ማድረጊያ ተግባር
...
def start(message):
    ...
    if user_id not in users_db:
        ...
        CHANNEL_ID =["@aa_11_b1","@ghhggghhhgg","@samiworkers",
 "@works_11w"] # እዚህ ላይ የቻናልህን username በ @ አስጀምረህ ጻፍ

        # በሪፈራል ሊንክ የገባ ከሆነ (ለምሳሌ /start 123456)
        ...
        text_message = f"👋 ሰላም {username}!\n\nቦቱን ለመጠቀም መጀመሪያ ሁሉንም ቻናሎቻችንን መቀላቀል አለብዎት።"
        bot.send_message(user_id, text_message, reply_markup=markup)

        return
        # 5ቱንም ቻናሎች አንድ በአንድ Join ማድረጋቸውን መፈተሻ
        not_joined = []
        for ch in CHANNELS:
            if not check_status(user_id, ch): # check_status ፋንክሽንህ ውስጥ ቻናሉን እንዲቀበል ማድረግ አለብህ
                not_joined.append(ch)
        
        # Join ያላደረጉት ቻናል ካለ ይህ ይነሳል
        if not_joined:
            markup = types.InlineKeyboardMarkup()
            
            # ለእያንዳንዱ ያልገቡበት ቻናል የመግቢያ በተን ይፈጥራል
            ...
            bot.send_message(
                user_id, 
                f"👋 ሰላም {username}!\n\nቦቱን ለመጠቀም መጀመሪያ ሁሉንም 5 ቻናሎቻችንን መቀላቀል አለብዎት።", 
                reply_markup=markup
            )
            return

    # ተቀላቅሎ ከሆነ ዋናውን ማውጫ ማሳየት
    # የሪፈራል ብር መስጠት (እስካሁን ካልተሰጠ)
    ref_id = users_db[user_id]['referred_by']
    if ref_id and users_db[user_id]['balance'] == 0.0 and users_db[user_id]['referred_count'] == 0:
        users_db[ref_id]['balance'] += REFERRAL_BONUS
        users_db[ref_id]['referred_count'] += 1
        users_db[user_id]['referred_count'] = -1 # ብሩ አንዴ ብቻ እንዲታሰብ ማድረጊያ
        try:
