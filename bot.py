...
CHANNEL_ID = "@ghhggghhhgg" # ቦቱ Admin መሆን አለበት!
ADMIN_ID = 8465808385
REFERRAL_BONUS = 3.00 # ለአንድ ሰው መጋበዣ የሚከፈለው የብር መጠን
MIN_WITHDRAW = 50.00  # ዝቅተኛው የማውጫ መጠን
# ---------------------
...
def start(message):
    ...
    if not check_status(user_id):
        ...
        markup.add(types.InlineKeyboardButton("📢 ቻናሉን ተቀላቀል", url=f"https://t.me/{CHANNEL_ID.replace('@', '')}"))
        markup.add(types.InlineKeyboardButton("✅ ተቀላቅያለሁ (Check)", callback_data="check_join"))
        bot.send_message(user_id, f"👋 ሰላም {username}!\n\nቦቱን ለመጠቀም መጀመሪያ ቻናላችንን መቀላቀል አለብዎት።", reply_markup=markup)
        return
